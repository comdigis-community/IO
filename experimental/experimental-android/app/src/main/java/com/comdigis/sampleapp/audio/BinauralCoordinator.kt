...
class BinauralCoordinator(
    ...
    init {
        ...
        // Build the graph through `Assemble` to keep this coordinator focused on
        // runtime behavior. The graph owns routing and destination output.
        ...
        try {
            ...
            // Initialize source and processing node together so the chain starts
            // in a coherent state before routing reaches the device output.
            ...
            // Build output routing in deterministic order for clarity and safe
            // processor replacement. Order: source -> binaural node -> destination.
    ...
    fun update(position: Vector3, forward: Vector3, up: Vector3, timestampSeconds: Double) {
        ...
        // deltas. The first update seeds zero velocity to avoid startup spikes.
    ...
    private fun configureSpatialSettings() {
        // Configure a neutral listener reference before UI orientation updates
        // begin, along with baseline physical defaults.
